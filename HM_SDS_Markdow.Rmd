...
=======
The idea is the following : We generate m $Unif(0,1)$ and then use them
>>>>>>> 3fd73e50134adf471505cd3d08926cd897c5baba
...
\~ $Unif(0,1)$ for each of them we generate 10 $Y$ \~ $Unif(0,1)$ and
check if there is a y greater than x, in case not we repeat the process
generating 4 times the lenght of Y. The idea of doing that is the
following:

Most of the time 10 simulations are enough to detect a y greater than the
x ( explanations are shown below) . If 10 simulations are not enough
probably the drawn x is big and such that we need to draw more Y.
=======
In oreder to simulate the actual process fastly, we first generate M $X$
\~ $Unif(0,1)$ for each of them we generate 10 $Y$ \~ $Unif(0,1)$ and
check if there is a y grater than x, in case not we repeat the process
generating 4 times the lenght of Y. The idea of doing that is the
following :

Most of the time 10 simulations are enough to detect a y grater than the
x ( exlpanation are shown below) . If 10 simulations are not enough
probably the drawn x is big and such of that we need to draw more Y.
>>>>>>> 3fd73e50134adf471505cd3d08926cd897c5baba

```{r simualtion 2, eval=FALSE}
...
<<<<<<< HEAD
Let's start the analysis with a fixed size M equal to 100000.
=======
Let's start the analysis with a fixed size M equal to 100000
>>>>>>> 3fd73e50134adf471505cd3d08926cd897c5baba
...
=======
    cond <- condition(runif(2) , val = x[m])
>>>>>>> 3fd73e50134adf471505cd3d08926cd897c5baba
    t <- cond + 1
    if (cond != 0)
      while(cond %% 10 == 0){
...
=======
First we comparare the simulation results with true function:
...
=======
Let's see the the series of the averages.
>>>>>>> 3fd73e50134adf471505cd3d08926cd897c5baba
...
impacts the approximation, to show that, we are going to
compute the distances between the true distribution and the estimated
distribution. For these analysis, we have used the geometric distribution,
=======
In this section we would quantitatively check how the simulation size
impacts the approximation, in order to show that, we are going to
compute the distances between the true distribution and the estimated
distibution. For these analysis we have used the geometric distribution,
>>>>>>> 3fd73e50134adf471505cd3d08926cd897c5baba
since returns the same result but computationally faster.
...
points(diff[6,] , col = "orange", type = "l")
```

[TODO] Redo the plots ![errors plot](Picture/error.png)
...
<<<<<<< HEAD
In this section, we are supposed to be a Casino that wants to introduce this
experiment in the available games. The player has to pay an amount of C
euros to start this game and he wins as many euros as the waiting time.

The question we are interested to answer is: What is the "fair" amount
to play this game?
=======
In this section we supposed to be a Casino that wants to introduce this
experiment in the available games. The player have to pay an amount of C
euros to start this game and he win as many euros as the waiting time.
...
Repeating this experiment many times, it will have to happen
(with very low probability) to win a lot, enough for paying all the
...
obtained at the beginning (a few units). Subsequently, the average rises,
=======
The mathematical result provides as a solution you can afford to pay any
amount C euros to play because on average you expect to win an infinite
amount.

In fact, repeating this experiment many time, it will have to happen
(with very low probability) to win a lot, enough for paying all the
expenses incurred before.
...
money to play this game. The intuitive refusal to invest large sums in
the game is well supported by the simulation described in the following
graph. By repeating the series 10,000 times, on average low winnings are
obtained at the beginning (a few units). Subsequently the average rises,
>>>>>>> 3fd73e50134adf471505cd3d08926cd897c5baba
corresponding to some lucky series, and then decreases slightly until
the next lucky hit. The overall trend is undoubtedly growing, and will
mathematically tend to infinity after an infinite series of plays but,
in the 10,000 plays of the simulation, the average of the winnings has
just reached the value of 10.
...
=======
From the statistical point of view, no difficulties came from the
situation presented in the game. In other words, it is perfectly
coherent to accept the possibility of an infinite win, such as to
>>>>>>> 3fd73e50134adf471505cd3d08926cd897c5baba
...
certain earnings..
=======
initial revenue or the amount you are willing to lose.

If the earnings here are "on average" infinite, you must also have
infinite funds and play an infinite number of times to be eligible for
certain earnings.
>>>>>>> 3fd73e50134adf471505cd3d08926cd897c5baba
...
Considering the fact of the maximum revenue fixed to L = 8 in the
>>>>>>> 3fd73e50134adf471505cd3d08926cd897c5baba
...
<<<<<<< HEAD
the decreasing function of m, because by adding more bins the histogram will be
an adequate representation of the true distribution.

In the case of n = 100 the blue line increase with large m, because we have
only 100 data to put inside 50 bins and may happen that a lot of  bins
are empty or with few units inside: this implied a more significant distance from
the two models.



=======
decreasing function of m , because adding more bins the histogram is a
good representation of the true distribution.

In case of n = 100 the blue line increase with large m, because we have
only 100 data to put inside 50 bins and may happen that a lot ob bins
are empty or with few units inside: this implied a greater distance from
the two models.
...
<<<<<<< HEAD
in the other hand, when $\epsilon$ too small we have a good privatization to
=======
in the other when $\epsilon$ too small we have a good privatization to
>>>>>>> 3fd73e50134adf471505cd3d08926cd897c5baba
the detriment of huge difference between the two distribution.
...
having university students fill out a form about the frequency of
their monthly workouts during 2022/2023. We collect 100
observations in order to study the distribution and statistics of this
feature that can be useful to understand physical health. Work-out
are often uncomfortable topics and since that we will share a
privatized data set built according to the perturbed histogram approach.
=======
The goal of this task is to privatized a data-set. We collect data by
having university students fill out a form about the frequencies of
their monthly work-out during the your 2022/2023. We collect 100
observations in order to study the distribution and statistics of this
feature that can be useful to understand the physical health. Work-out
are often an uncomfortable topic and since that we will share a
privatized data-set built according to the perturbed histogram approach.
>>>>>>> 3fd73e50134adf471505cd3d08926cd897c5baba
