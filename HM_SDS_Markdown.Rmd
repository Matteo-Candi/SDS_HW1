---
title: "Homework 1 of Statistical for Data Science"
...
In this section we would quantitatively check how the simulation size impacts the approximation, in order to show that, we are going to compute the mean error distances between the true distribution and the estimated distibution.

For each simulation size we compute the mean distance over the set 1:25.

```{r simualtion analysis}
Sim <- c(100, 1000, 10000, 100000, 1000000, 10000000 )  # vector of the size 
...
for (j in 1:M){
  ...
  for (s in Sim){    
    ...
    diff[i,j] <- mean(pT(t_seq) - p_hat[t_seq], na.rm = T)  #compute the mean distance
    i <- i + 1
}
}
```
